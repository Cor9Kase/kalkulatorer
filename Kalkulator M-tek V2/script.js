...
function handleNextStep() {
    if (isResultTransitioning || !elements.nextBtn) return;

    isResultTransitioning = true;
    setButtonLoadingState(elements.nextBtn, true);
    ...
    setTimeout(() => {
        ...
        if (elements.quoteSuccess) {
            elements.quoteSuccess.classList.add('hidden');
        }

        updateDisplay();
        elements.resultsSection.scrollIntoView({ behavior: 'smooth' });
        setButtonLoadingState(elements.nextBtn, false);
...
function handleQuoteSubmit(e) {
    ...
    if (elements.quoteSuccess) {
        elements.quoteSuccess.classList.remove('hidden');
    }
